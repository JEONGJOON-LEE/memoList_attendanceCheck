...
//  memoList.jsp에서 넘어오는 수정할 글번호(idx)와 수정 후  돌아갈 페이지 번호(currentPage)를 받는다
	String tempIdx = request.getParameter("idx");
	String tempCurrentPage = request.getParameter("currentPage");
	
//	수정할 글번호와 돌아갈 페이지 번호가 모두 넘어왔나 검사한다.
	if(tempIdx != null && tempCurrentPage != null && (tempIdx.trim().length() + tempCurrentPage.trim().length() != 0)){
		
//		수정할 글번호와 돌아갈 페이지 번호가 숫자인가 검사한다.
		try{
			int idx = Integer.parseInt(tempIdx);
			int currentPage = Integer.parseInt(tempCurrentPage);
			
//			수정할 글 번호에 해당되는 글이 테이블에 존재하는가 확인하기위해서 수정할 글번호의 해당되는 글을 얻어온다.
...
<!-- 수정할 글이 테이블에 존재하므로 수정할 글을 화면에 보여주고 비밀번호를 입력받는다. -->				
	<form action="updateOK.jsp" method="post">
		<table width="700" align="center" border="1" cellpadding="5" cellspacing="0">
			...
			<tr>
				...
				<td>
					...
					<input type="text" name="memo" value="<%=rs.getString("memo")%>" style="width: 95%;"/>
				</td>
			</tr>
		...
		<!-- 수정할 글 번호와 수정 후 돌아갈 페이지 번호도 updateOK.jsp로 넘겨줘야 한다. -->
...
//			수정할 글 번호와 돌아갈 페이지 번호가 숫자가 아님으로 에러 메시지를 출력하고 memoList4.jsp로 보낸다.	
			out.println("<script>");
...
//		수정할 글 번호와 돌아갈 페이지 번호가 모두 넘어오지 않았으므로 에러 메시지를 출력하고 memoList4.jsp로 보낸다.	
		out.println("<script>");
